...
for line in data:
    ...
    if line == "":
        continue
    line = line.split("\t")
    hid_code = int(line[0], 16)
    key_name = line[1]
    # remove the 'Keyboard ' prefix
    key_name = key_name.replace("Keyboard ", "")
    key_name = key_name.replace("Keyboard,", ",")
    key_name = key_name.replace("Keypad ", "keypad_")
    if ' and' in key_name:
        key_name = key_name.split(" and")
        key_name = key_name[0]
    print(f"\"{key_name}\" : {hid_code},")